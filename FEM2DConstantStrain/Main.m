...
[dn,U,F,mp,nc,nd,ndim,ndn,ne,nen,nl,nm,nn,np,x,TH,MAT,nq,LC]...
    = readData(finp);
% kc = zeros(6,2);
% cn = nc.';
% for i = 1:6
...
kc = zeros(6,2);
cn = nc.';
[DA, K,DB] = dbmat(ne, LC, MAT, mp, nc, x,K,kc,cn);
...
%         C = 0;
%         for k = 1:3
%             C = C +0.5*abs(DA)*B(k,i)*DB(k,j);
%         end
%         K(i,j) = C;
%     end
% end

% [K,k]=stiffness(nn,mm,ne,nc,MAT,x,mp,TH);
% 
% %----------------------------------------------
