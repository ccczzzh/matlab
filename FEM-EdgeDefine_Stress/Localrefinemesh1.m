function [Gcoord,nodemap] = Localrefinemesh1(lfn,lne,nodemap,Gcoord,localx,localy,eltype,ndim,nn)
% Local coordinate matrix.

Lcoord = zeros(localx*localy,2);
x = Gcoord(nodemap(lfn,:),:);
...
% -------------------------------------------------------------------
% Append the local coordinate behind Global coordinate
Gcoordreduced = Gcoord; %Gcoordreduced(nodemap(nn,:),:)=[];
    ...
    for layer = 1:(localy-1)
        if layer == 1
            if inen == 1
                ...
                localnodemap(1+(layer-1)*(localx-1),:) =  [nodemap(lfn,1), nn+1, nn+1+localx-1,nn+1+localx-2]; % 6 17 20 19
            elseif inen == (localx-1)
                % last node of first layer
                localnodemap(localx-1+(layer-1)*(localx-1),:) =  [nn+2,nodemap(lfn,2), nn+1+localx+1,nn+1+localx]; % 18 7 22 21
            else
                localnodemap(inen+(layer-1)*(localx-1),:) =  [nn+1, nn+2, nn+1+localx-2+2,nn+1+localx-2+1];
            end
        elseif layer == (localy-1)
            if inen == 1
                localnodemap((localx-1)*(localy-2)+1,:) = [nn+2+(layer-2)*localx+1, nn+2+(layer-2)*localx+2, nn+2+(layer-1)*localx+1, nodemap(lfn,4)];
            elseif inen == (localx-1)
                localnodemap((localx-1)*(localy-1),:) = [nn+2+(layer-2)*localx+3, nn+2+(layer-1)*localx, nodemap(lfn,3),nn+2+(layer-1)*localx+2];
            else
...
% nodemap1(n,:) =  [nodemap(nn,i), nn+1, nn+1+localx-1,nn+1+localx-2];
nodemapreduce = nodemap;nodemapreduce(lfn,:)=[]; % remove the global node matrix at the local refine area
